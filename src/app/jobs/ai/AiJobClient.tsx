...
import { AssignmentFields, type AssignmentValue } from '@/components/jobs/AssignmentFields'
...
export default function AiJobClient({ workspaces, currentUserId }: Props) {
  ...
  const [assignment, setAssignment] = useState<AssignmentValue>({
    groupId: '', ownerUserId: currentUserId, assignedTo: '', assignedTeamId: '',
  })
  const [assignmentInitial, setAssignmentInitial] = useState<Partial<AssignmentValue> | undefined>(undefined)
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      // Seed assignment with the workspace the AI matched, if any
      const matched = ai.matchedGroupId && workspaces.find(w => w.id === ai.matchedGroupId)
      setAssignmentInitial({
        groupId:        matched ? matched.id : '',
  ...
  async function handleConfirm() {
    ...
    if (workspaces.length > 0 && !assignment.groupId) {
    ...
    try {
      const res = await fetch('/api/ai/create-job', {
        ...
        body: JSON.stringify({
          ...
          assigned_team_id: assignment.assignedTeamId || null,
        }),
      })
      const data = await res.json()
  ...
  const matchedWorkspace = result.matchedGroupId && workspaces.find(w => w.id === result.matchedGroupId)
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            {matchedWorkspace && (
              ...
              >
                AI matched: {matchedWorkspace.name}
              </span>
            )}
          </div>
          <AssignmentFields
