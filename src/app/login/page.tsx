  ...
  return (
    <div className="flex items-center gap-2.5 mb-8">
      ...
      >
        ...
        />
        <polygon
          points="16,10 25,25 7,25"
          fill="#0F1117"
        />
        <polygon
          points="16,14 22,24 10,24"
          fill="#C8A44E"
          opacity="0.5"
        />
      </svg>
...
// ── URL error (from redirect params) ─────────────────────────────────────────

function UrlError() {
  const params = useSearchParams()
  const error  = params.get('error')
  if (!error) return null
  return <ErrorBanner message={decodeURIComponent(error)} />
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* URL-level errors (e.g. Google OAuth failure) */}
        <Suspense fallback={null}>
          <UrlError />
        </Suspense>

        {/* Action-level errors (email/password failures) */}
